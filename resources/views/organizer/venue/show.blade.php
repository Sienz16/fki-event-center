<x-organizer-layout>
    ...
    <div class="min-h-[calc(100vh-65px)] pb-8" x-data="calendarModal()">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        @if($venue->equipment)
                                            <ul class="list-disc list-inside">
                                                @foreach(explode(',', $venue->equipment) as $equipment)
                                                    <li>{{ trim($equipment) }}</li>
                                                @endforeach
                                            </ul>
                                        @else
                                            <span class="text-gray-500 italic">No equipment listed</span>
                                        @endif
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div class="grid grid-cols-7 gap-2 text-center text-gray-900 font-medium mb-4">
                    ...
                    <template x-for="day in daysInMonth" :key="day">
                        <div class="p-2 rounded relative transition duration-200 ease-in-out" x-data="{ showTooltip: false }"
                             :class="hasEvent(day) ? 'bg-purple-200 rounded-lg' : ''"
    ...
    <script>
        function calendarModal() {
            return {
                ...
                getEventsForDay(day) {
                    // Create a currentDate with time set to midnight to avoid timezone issues
                    ...
                    return this.events.filter(event => {
                        ...
                        if (event.event_date) {
                            ...
                            eventDate.setHours(0, 0, 0, 0); // Ensure event date is at midnight for comparison
                            eventDateMatch =
